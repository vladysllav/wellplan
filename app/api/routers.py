...
from app.schemas.doctor import CreateDoctor, DoctorUpdate, CreateBranch, BranchUpdate
...
@api_router.post("/create_doc/") #response_model=Doctor
...
def get_doctor_list(db: Session = Depends(get_db)):
    doctor = crud_doctor.get_multi(db)
    return doctor
...
@api_router.put("/update_doc/{doctor_id}") #response_model=Doctor
...
@api_router.delete("/delete_doc/{doctor_id}") #response_model=Doctor
