...
#     user_info_from_db = crud_user.get_user(db, current_user.id)  # Получите информацию о пользователе из базы данных
...
def update_user(
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    updated_user = crud.user.update_user(db, user, user_in)
    return updated_user
