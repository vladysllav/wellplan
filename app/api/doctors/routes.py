...
from app.api.deps import get_db, get_current_user, check_admin
...
def get_doctor_list(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def delete_doctor(doctor_id: int, db: Session = Depends(get_db), user: User = Depends(check_admin)):
...
def get_branches_list(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def get_branches(branch_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def delete_branch(branch_id: int, db: Session = Depends(get_db), user: User = Depends(check_admin)):
    branches = crud_branch.get(db, branch_id)
    if branches is None:
        raise HTTPException(status_code=404, detail="Branch not found")
